...
Because `musl` cannot compile `libpcap`, and the results compiled with `gnu` cannot be migrated to different Linux distributions, the download interface only provides downloads of musl based on `libpnet` by default. If you want to use `widump` based on `libpcap`, please use the following method to install it.
...
```bash
cargo install widump --features "libpcap"
```
...
```bash
widump -w widump.pcapng
```

Or specify interface.

```bash
widump -i ens33 -w widump.pcapng
```

Capture the traffic and apply filter.

```bash
widump -i ens33 -w widump.pcapng -f 'tcp and (ip=192.168.1.1 or ip=192.168.1.2) and dstport=80'
```

Capture the traffic and split according to time.

```bash
widump -i ens33 -w widump.pcapng --rotate 60s
```

Capture the traffic and split according to file size.

```bash
widump -i ens33 -w widump.pcapng --file-size 10M
```

Capture the traffic and split according to packet count.

```bash
widump -i ens33 -w widump.pcapng --count 1024
```
...
```bash
widump --mode client -i ens33 --server-addr '127.0.0.1:12345'
```
...
```bash
widump --mode server --server-addr '127.0.0.1:12345' --rotate 1h
```

Or

```bash
widump --mode server --server-addr '127.0.0.1:12345' --file-size 100M
```

Or

```bash
widump --mode server --server-addr '127.0.0.1:12345' --count 1024
```